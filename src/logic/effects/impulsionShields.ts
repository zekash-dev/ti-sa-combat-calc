...
export const impulsionShields: ParticipantTagImplementation = {
    preAssignHits: ({ role, units, hits, tagState }: PreAssignHitsInput): PreAssignHitsOutput => {
        ...
        for (let hitType of negateHitPriorityOrder) {
            ...
            if (determineHitTarget(units, hitType) === -1) continue; // Hit of type can't be assigned to any of your units
            return {
